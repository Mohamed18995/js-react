...
BTN_VALIDER.addEventListener('click', function (MouseEvent) {
  ...
  const URL = `http://localhost/cours1/chapitre12-ajax%20-%20Copie/test_ajax.php`;
...
function ajax_film(URL){
  ...
  function traitement_film(film){
    ...
    if (film.titre && film.pays && film.duree && film.date){
      ...
      BUTTON.value = film.titre
      BUTTON.className = "info-button"
      BUTTON.addEventListener("click", function (){
        console.log(film.pays, film.titre, film.duree, film.date);
        // alert(film.pays +" "+ film.titre +" " + film.duree +" " + film.date)
      })
